...
- [x] **S2. Enforce foreign keys** (server C4, S): `foreign_keys = ON` enabled at startup **only** behind a read-only orphan-audit gate (fail-open if orphans exist). New `server/orphan-audit.ts` + `scripts/audit-orphans.mjs` (Render-Shell runnable) + 4 tests. PR #78, merge `fc83e41`, deployed 2026-07-19. **Verified live in production 2026-07-19 12:44 MDT:** Kevin ran `scripts/audit-orphans.mjs` in the Render Shell (0 orphans across all 10 relationships) and restarted the service; startup log shows `FK enforcement ENABLED (foreign_keys = ON) — orphan audit clean`.
