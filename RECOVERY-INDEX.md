...
| Date       | Branch                              | Summary |
| ---------- | ----------------------------------- | ------- |
| 2026-07-19 | (Phase 1 prep: Stage A + A2, PRs #94–#98) | **Stage A (async storage) + A2 single source of truth — closes Issue #64.** Five sequential production deploys finishing the legacy dual-storage problem; each `check`/`test`/`build` PASS, CI green, prod HTTP 200 before the next. Tests 56 → 84; no schema migration in any PR. **Stage A** `c253bf7` (PR #94): storage interface made async (~50 `IStorage` methods promise-based, 123 awaited call sites, 88 async route handlers; `verifyPassword` deliberately kept sync for auth safety; SQLite behavior unchanged). **Audit** `aab9518` (PR #95): read-only `scripts/audit-legacy-divergence.mjs`; production run = 76 calls, 15 diverged, 0 review flags, 1 missing a Product 1 row. **A2-1** `1ac8023` (PR #96): `updateServiceCall` write-through also writes Product 1 (creates if missing), idempotent for offline replay (6 tests). **A2-2** `8f12eba` (PR #97): gated `scripts/reconcile-legacy-product1.mjs` — dry-run default; `--apply` archives every overwritten value to `/var/data/reconcile-archive-<ISO>.json` first, single transaction, re-verifies 0 diverged, idempotent (9 tests). **Kevin ran the reconcile in production 2026-07-19 ~5:25 PM MDT:** backup taken (`manual-backup-*.db`), 15 updates + 1 create applied, archive `reconcile-archive-2026-07-19T23-24-30.256Z.json` written, post-apply + re-audit both **0 diverged**. **A2-3** `d49fc48` (PR #98, closes #64): all readers resolve the 16 legacy fields from Product 1 with legacy fallback (`pickProduct1`/`overlayProduct1`; `globalSearch` via `COALESCE(NULLIF(TRIM(...)))`, also fixing a latent stale-equipment-search bug); `syncLegacyFromProduct` removed; legacy columns kept as a live mirror (dropped later in Stage B / Postgres); 6 parity tests. Landed **only after** the production reconcile + clean re-audit, so readers are byte-identical on reconciled data. Rollback anchor before Stage A: `6662465`, or the prior merge in-chain. **Recovery artifacts on `/var/data`:** the pre-reconcile `manual-backup-*.db` and `reconcile-archive-2026-07-19T23-24-30.256Z.json`. |
...
**Applied in production 2026-07-19 ~5:25 PM MDT** (Kevin, Render Shell): manual
backup taken, 15 updates + 1 create applied, post-apply verification and the
re-audit both reported 0 diverged. After the reconcile, PR #98 (A2 step 3) made
Product 1 the single source of truth and removed `syncLegacyFromProduct` — this
closed **Issue #64**.

**Recovery artifacts (on the `/var/data` persistent disk):** if the reconcile
ever needs to be undone, two artifacts from that run are the recovery sources —
the pre-reconcile `manual-backup-*.db` (full DB copy taken in step 1) and
`reconcile-archive-2026-07-19T23-24-30.256Z.json` (the JSON archive of every
Product 1 value the `--apply` overwrote, plus the full plan). Both sit beside the
live DB under `/var/data`.
